...
<Tabs>
<Tab title="Per-Action Accounting" icon="calculator">

For LAM version `v` with USD price per action `pᵥ`:
...
- Burn: `Bₐct = β × τᵥ,ₜ` where β = 0.34
- Creator: `Mₐct = μ × τᵥ,ₜ × 𝟙marketplace` where μ = 0.33
- Foundation: `Fₐct = φ × τᵥ,ₜ` where φ = 0.33

Where:
- `τᵥ,ₜ` = Tokens required per action at time t
- `Pₜ` = Market price of $LAM in USD at time t
- `𝟙marketplace` = 1 if marketplace workflow, 0 otherwise

</Tab>
<Tab title="Network-Level Dynamics" icon="network-wired">

### Period t Calculations (e.g., monthly)

Let `Aₜ` be total actions executed in period t:

**Token Flows:**
```
Tokens needed:     Dₜ = τᵥ,ₜ × Aₜ
Burned tokens:     Bₜ = β × Dₜ
Creator tokens:    Mₜ = μ × Dₜ × Pr(marketplace)
Foundation tokens: Fₜ = φ × Dₜ
USD spent:         Uₜ = pᵥ × Aₜ (price-invariant)
```
...
Where:
- `Sₜ` = Circulating supply at start of period t
- `Eₜ` = New emissions (training rewards, grants)
- `Bₜ` = Tokens burned in period t

</Tab>
<Tab title="Equilibrium Analysis" icon="balance-scale">
...
**Half-life under constant burn (no emissions):**
```
T₁/₂ = S₀ / (2 × β × τᵥ,ₜ × Aₜ)
```

Where `S₀` is initial supply and actions are steady.
